...
const SUSPEND_WARN_TITLE = "Saving your ass!";
const SUSPEND_WARN_BODY = "Suspending.Please recharge me";
...
async function batteryMessage() {
    ...
    if (perc <= SUSPEND_THRESHOLD && !charging) {
        Utils.execAsync(['bash', '-c',
            `notify-send '${SUSPEND_WARN_TITLE}' '${SUSPEND_WARN_BODY}' -u critical -a "ags" & sleep 3; systemctl suspend`
        ]).catch(print);
        return;
    }
